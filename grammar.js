...
module.exports = grammar({
  ...
  rules: {
    ...
    _event_parameter_list: ($) => seq('(', commaSep($.event_paramater), ')'),
    ...
    assembly_statement: ($) =>
      seq(
        'assembly',
        optional($._assembly_flags),
        '{',
        repeat($._yul_statement),
        '}'
      ),

    _assembly_flags: ($) => seq('(', commaSep($.string), ')'),
    ...
    block_statement: ($) =>
      seq(optional($._unchecked), '{', repeat($._statement), '}'),
    variable_declaration_statement: ($) =>
      prec(
        1,
        seq(
    ...
    if_statement: ($) =>
      prec.right(
        seq(
          ...
          field('else', optional(seq('else', field('body', $._statement))))
        )
      ),
    ...
    return_statement: ($) =>
      seq('return', optional($._expression), $._semicolon),
    ...
    call_argument: ($) =>
      choice($._expression, seq('{', commaSep($.call_struct_argument), '}')),
    ...
    type_cast_expression: ($) =>
      prec.left(seq($.primitive_type, '(', $._expression, ')')),
    ...
    tuple_expression: ($) =>
      prec(1, seq('(', commaSep(optional($._expression)), ')')),
    ...
    call_expression: ($) =>
      prec.right(
        PREC.CALL,
        seq(field('function', $._expression), $._call_arguments)
      ),
    ...
    _array_type: ($) =>
      prec(1, seq($.type_name, '[', optional($._expression), ']')),
